...
f = ROOT.TFile.Open('/data/bhollo/mgNLO/photonModeling/tqgammaNLO.root', 'r')
t = f.Get("selection")
tw = f.Get("totalWeight").GetBinContent(1)
ew = 1.0/(tw)

for var in variables:
    c1 = TCanvas('c1', ' ', 10000, 1000)

    hNtqg = TH1D('hNtqg', 'hNtqg', var[2], var[3], var[4])
    hNtqg.Sumw2()
    t.Draw("%s >> hNtqg" %var[0], "%.10f*weight" %ew)

    leg = TLegend(0.63, 0.60, 0.78, 0.75)
    leg.AddEntry(hNtqg, "NLOtqgamma")

    hNtqg.SetLineColor(ROOT.kViolet)
    hNtqg.GetXaxis().SetTitle(var[1])
    nevents = (1.1802)*(1.39e+05) #xsec(pb)*luminosity(pb^-1)
    hNtqg.Scale(nevents)
    hNtqg.Draw('hist')
    leg.Draw()

    c1.SaveAs('NLOtqGammaPlots/'+var[5]+'.png')
